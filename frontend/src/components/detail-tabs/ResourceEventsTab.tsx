...
export const ResourceEventsTab = memo(function ResourceEventsTab({
  namespace,
  resourceName,
    ...
    <div className="bg-surface-light dark:bg-surface-dark rounded-lg border border-border-light dark:border-border-dark overflow-hidden">
      ...
      {events.length === 0 ? (
        <div className="px-6 py-12 text-center">
          <span className="material-symbols-outlined text-4xl text-slate-400 dark:text-slate-600 mb-2 block">event_busy</span>
            ...
            {events.map((event, i) => {
              ...
              return (
                <tr key={event.metadata?.name ?? i} className="hover:bg-slate-50 dark:hover:bg-surface-hover/30">
                  ...
                  <td className="px-6 py-3 text-slate-400 max-w-md truncate">{event.message ?? '-'}</td>
                  <td className="px-6 py-3 font-mono text-xs whitespace-nowrap">{ts ? relativeTime(ts) : '-'}</td>
