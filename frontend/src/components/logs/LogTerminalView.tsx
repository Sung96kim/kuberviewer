...
function createTerminal(el: HTMLElement) {
...
export const LogTerminalView = memo(function LogTerminalView({
  ...
  useEffect(() => {
    ...
    const { terminal, fitAddon, searchAddon } = createTerminal(el)
  ...
  return (
    <div className="flex flex-col h-full min-h-0 rounded-lg overflow-hidden bg-[#0f172a] border border-slate-700">
      <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-900/80 border-b border-slate-700 shrink-0">
        <span className={`inline-block h-2 w-2 rounded-full shrink-0 ${logHook.connected ? 'bg-emerald-500' : 'bg-slate-500'}`} />
        <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider shrink-0">Logs</span>

        {containers.length > 1 && (
          <select
            value={selectedContainer}
            onChange={(e) => onContainerChange(e.target.value)}
            className="ml-1 text-xs bg-slate-800 text-slate-300 border border-slate-600 rounded px-1.5 py-0.5 focus:outline-none focus:ring-1 focus:ring-primary"
          >
            {containers.map((c) => (
        ...
        <div className="ml-auto flex items-center gap-1 shrink-0">
          ...
          <button
            ...
            className={`p-1 rounded transition-colors ${follow ? 'text-emerald-400 hover:bg-slate-800' : 'text-slate-500 hover:bg-slate-800'}`}
      ...
      {searchOpen && (
        <div className="flex items-center gap-1.5 px-2 py-1 bg-slate-800/90 border-b border-slate-700 shrink-0">
          ...
          />
          <button onClick={searchPrev} className="p-0.5 rounded text-slate-400 hover:text-slate-200 hover:bg-slate-700 transition-colors" title="Previous (Shift+Enter)">
            <span className="material-symbols-outlined text-[14px]">expand_less</span>
          </button>
          <button onClick={searchNext} className="p-0.5 rounded text-slate-400 hover:text-slate-200 hover:bg-slate-700 transition-colors" title="Next (Enter)">
            <span className="material-symbols-outlined text-[14px]">expand_more</span>
          </button>
          <button onClick={closeSearch} className="p-0.5 rounded text-slate-400 hover:text-slate-200 hover:bg-slate-700 transition-colors" title="Close (Esc)">
      ...
      {logHook.error && (
        <div className="px-3 py-1.5 text-xs text-red-400 bg-red-500/10 border-b border-slate-700 shrink-0">{logHook.error}</div>
      )}
