...
export const TerminalDrawer = memo(function TerminalDrawer() {
  ...
  return (
    <div
      className="fixed bottom-0 left-0 right-0 z-50 bg-[#0f172a] border-t border-slate-700 flex flex-col"
      style={{ height }}
    >
      <div
        className="h-1.5 cursor-row-resize hover:bg-primary/30 transition-colors flex-shrink-0"
        onMouseDown={handleMouseDown}
      >
        <div className="w-10 h-0.5 bg-slate-600 rounded mx-auto mt-0.5" />
      </div>

      <div className="flex items-center gap-1 px-2 py-1 bg-slate-900/80 border-b border-slate-700 flex-shrink-0 overflow-x-auto">
          ...
          >
            {group.panes.map((pane, index) => (
              <div
                ...
                style={{ borderLeft: index > 0 ? '1px solid rgb(51 65 85)' : undefined }}
                onClick={() => setActivePane(group.id, pane.id)}
              >
                {group.panes.length > 1 && (
                  <div className="flex items-center justify-between px-2 py-0.5 bg-slate-900/60 border-b border-slate-700/50 flex-shrink-0">
                    ...
                    <button
                      ...
                      className="p-0.5 rounded hover:bg-slate-800 text-slate-500 hover:text-red-400 transition-colors"
                    >
